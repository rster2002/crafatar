...
// 37faca995aa2bfa391f1c9ddcb20118fdc331bf5c8a5172bb4c7eb038e3d2b2c
var valid_skin_hash = /[0-9a-f]{33,64}/;
...
exp.is_skin_hash = function (skin_hash) {
  return valid_skin_hash.test(skin_hash);
}
...
exp.get_image_hash = function(rid, userId, type, callback) {
  if (exp.is_skin_hash(userId)) {
    callback(null, 1, userId, true);
  }
...
exp.get_render = function(rid, userId, scale, slimOverride, overlay, body, callback) {
  exp.get_skin(rid, userId, function(err, skin_hash, status, img, slim) {
    if (slimOverride !== null) {
      slim = slimOverride;
    }
