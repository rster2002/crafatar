...
module.exports = function(req, callback) {
  ...
  var overrideSlim = req.url.query.slim !== undefined; // Override for when requesting via texture hash
  var slim = overrideSlim ? req.url.query.slim === "1" : null;
  ...
  if (scale < config.renders.min_scale || scale > config.renders.max_scale) {
    ...
    return;
  } else if (!helpers.id_valid(userId) && !helpers.is_skin_hash(userId)) {
    callback({
      status: -2,
      body: "Invalid UUID"
    });
  ...
  try {
    helpers.get_render(rid, userId, scale, slim, overlay, body, function(err, status, hash, image) {
      if (err) {
        if (err.code === "ENOENT") {
          // no such file
